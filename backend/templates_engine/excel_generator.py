...
def generate_excel_report(report_type, summary_data, df_data):
    ...
    if summary_data:
        ws.cell(row=current_row, column=1, value="Key Performance Indicators (KPIs)").font = bold_font
        current_row += 1
        ...
        for key, value in summary_data.items():
            ...
            ws.cell(row=current_row, column=1, value=display_key).font = normal_font
            ...
            if isinstance(value, (int, float)):
                val_cell.number_format = '#,##0.00' if isinstance(value, float) else '#,##0'
    ...
    if not df_data.empty:
        ws.cell(row=current_row, column=1, value="Detailed Data Records").font = bold_font
        current_row += 1
